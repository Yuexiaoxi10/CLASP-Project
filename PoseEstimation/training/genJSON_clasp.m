function joint_all = genJSON_clasp(dataset,my_data)
    addpath('../../testing/util');
    addpath('../../testing/util/jsonlab/');

    if(strcmp(dataset, 'CLASP'))
        %mkdir('./Clasp/json')
        count = 1;
        makeFigure = 0;
        validationCount = 0;
        ...
        for mode = 0%:1
            ...
            % clasp joint order: 1, 0,0,0,0,6,8,10,7,9,11,0,0,0,0,0,0 
            ...
            for i = trainIdx
                ...
                for p = 1:numPeople
                    h = RELEASE(i).annorect(p).img_height;
                    w = RELEASE(i).annorect(p).img_width;
                    ...
                    if mode == 0
                        joint_all(count).dataset = 'clasp';
                    %else
                        %joint_all(count).dataset = 'clasp_c11';
                    end
                    ...
                    % set image path
                    if mode == 0
                        joint_all(count).img_paths = RELEASE(i).filepath;
                    ...
                    if(makeFigure) % visualizing to debug
                        ...
                        imshow([imroot, cell2mat(joint_all(count).img_paths)]);
        ...
        opt.FileName = './Clasp/json/Clasp_5A_9A_exp4.json';
        opt.FloatFormat = '%.3f';
